...
a time, reduce each block to its ``(block,)`` log-probs, and free the
``(block, vocab)`` logits before the next block. Peak logits memory is
``chunk_size * vocab``, independent of ``L``. It is pure PyTorch (no extra deps,
runs on CPU), numerically identical to the naive path, and gradient-exact, so it
drops into both the no-grad reference/old passes and the grad-carrying update.
"""
